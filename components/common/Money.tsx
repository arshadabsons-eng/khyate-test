import { aed, filsToAed, aedCompact } from "@/lib/format";

// The official UAE Dirham symbol (2025), inlined as SVG so it inherits the
// current text colour and scales with font-size. Use instead of "AED"/"Dh".
export function Dh({ className = "" }: { className?: string }) {
  return (
    <svg
      ...
      role="img"
      aria-label="AED"
      style={{ height: "0.82em", width: "auto", verticalAlign: "-0.04em", flexShrink: 0 }}
      className={`dh-symbol ${className}`}
    >
...
// Money amount = Dirham symbol + formatted number. Pass `fils` (integer fils),
// `aed` (already in dirhams), and `compact` for short forms (12k / 1.4m).
export function Money({
  ...
  const text = compact
    ? aedCompact(fils ?? (aedVal != null ? aedVal * 100 : 0))
    : fils != null
      ? filsToAed(fils)
      : aed(aedVal ?? 0);
  return (
